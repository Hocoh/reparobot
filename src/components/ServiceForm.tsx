...
const ServiceForm = ({
  ...
  const validateForm = () => {
    ...
    service.formFields.forEach((field) => {
      ...
      if (field.type === 'tel' && formValues[field.label] && !phoneRegex.test(formValues[field.label])) {
        newErrors[field.label] = `${field.label} doit être un numéro de téléphone valide`;
      }
    });
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    service.formFields.forEach((field) => {
      if (field.type === 'date' && formValues[field.label]) {
        formattedValues[field.label] = dayjs(formValues[field.label]).format('DD/MM/YYYY');
      }
    });
      ...
      case 'checkbox_price':
        return (
          ...
          >
            ...
            />
            {!!errors[field.label] && (
              <FormHelperText id={`${field.label}-error`}>{errors[field.label]}</FormHelperText>
            )}
          </FormControl>
        );
      case 'date':
